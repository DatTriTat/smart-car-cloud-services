...
const { parseArrayParam, parseIntParam, parseDateParam } = require("../helpers/queryParser");

class AlertController {
  async create(request, response) {
    const { carId, classifiedResults, metadata, audioEventId } = request.body;
    if (!carId) throw new BadRequestError("Missing carId");

    const data = await AlertService.createAlert({ carId, classifiedResults, metadata, audioEventId });
    return new CREATED({ message: "Alert created successfully", data }).send(response);
  }
  ...
  async getAlerts(request, response) {
    ...
    return new OK({ message: "Alerts retrieved successfully", data: result }).send(response);
  }

  async getAlert(request, response) {
    const { alertId } = request.params;
    if (!alertId) throw new BadRequestError("Missing alert id");

    const alert = await AlertService.getAlertById(alertId);
    return new OK({ message: "Alert retrieved successfully", data: alert }).send(response);
  }
  ...
  async updateStatus(request, response) {
    ...
    if (!status || typeof status !== "string") throw new BadRequestError("status is required");

    if (!userId && request.user && request.user.id) userId = request.user.id;

    status = status.toLowerCase().trim();

    const updated = await AlertService.updateStatus(alertId, userId || null, status, comment || null);

    return new OK({ message: "Alert status updated successfully", data: updated }).send(response);
  }
}
